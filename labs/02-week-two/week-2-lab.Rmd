...
p_count <- ggplot(data = loans, aes(x = homeownership)) +
  geom_bar(fill = IMSCOL["green", "full"]) + # `IMSCOL[]` from package `openintro` for textbook - nice colors
...
p_stacked <- ggplot(data = loans, aes(x = homeownership, fill = application_type)) +
  geom_bar() # Look at answer key to see how to make this graph prettier
p_stacked
```
